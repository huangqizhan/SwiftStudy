...
// MARK: 首页推荐接口
let HHomeRecommondProvider = MoyaProvider<HHomeRecommondApi>()

enum HHomeRecommondApi{
    ///推荐列表
    case recommondList
    ///为你推荐
    case recommondForyouList
    ///推荐页面广告
    case recommondAdverList
    ///猜你喜欢更多
    ...
    case changeLiveList
    ///更换其他
    case changeOtherCategory(cateId : Int)
}


extension HHomeRecommondApi : TargetType{
    var baseURL: URL{
        switch self {
        case .recommondAdverList:
    ...
    var task: Task{
        ...
        case .changeOtherCategory(let cateId):
            parmeters = [
                "appid":0,
            ...
            parmeters["categoryId"] = cateId
        }
        return Task.requestParameters(parameters: parmeters, encoding: URLEncoding.default)
    }
